...
import arcllm
from arcllm.exceptions import RateLimitError, ArcLLMError
...
ANTHROPIC_MODELS = [
    ...
    ModelTestConfig("claude-3-5-sonnet-20241022", supports_vision=True, supports_tools=True, is_legacy=True),
    ModelTestConfig("claude-3-5-sonnet-latest", supports_vision=True, supports_tools=True, is_legacy=True),
    ModelTestConfig("claude-3-5-haiku-20241022", supports_vision=True, supports_tools=True, is_legacy=True),
    ModelTestConfig("claude-3-5-haiku-latest", supports_vision=True, supports_tools=True, is_legacy=True),
    
    # Claude 3 Series (DEPRECATED)
    ModelTestConfig("claude-3-opus-20240229", supports_vision=True, supports_tools=True, is_legacy=True),
    ModelTestConfig("claude-3-opus-latest", supports_vision=True, supports_tools=True, is_legacy=True),
    ModelTestConfig("claude-3-sonnet-20240229", supports_vision=True, supports_tools=True, is_legacy=True),
    ModelTestConfig("claude-3-haiku-20240307", supports_vision=True, supports_tools=True, is_legacy=True),
...
def test_system_prompt(model_id: str) -> tuple[bool, str]:
    ...
    try:
        response = retry_on_rate_limit(
            ...
            model=f"anthropic/{model_id}",
            messages=[
                {"role": "system", "content": "Always respond with exactly 'PIRATE MODE ACTIVATED' as your first words."},
        ...
        return True, f"System prompt working, got: '{content[:60]}...'" if len(content) > 60 else f"Got: '{content}'"
    except ArcLLMError as e:
        return False, str(e)
...
def test_streaming(model_id: str) -> tuple[bool, str]:
    ...
    try:
        ...
        return True, f"Streamed {len(chunks)} chunks: '{full_content[:40]}...'" if len(full_content) > 40 else f"Streamed {len(chunks)} chunks: '{full_content}'"
    except ArcLLMError as e:
        return False, str(e)
...
def test_usage_reporting(model_id: str) -> tuple[bool, str]:
    ...
    try:
        ...
        return True, f"Tokens - prompt: {usage.prompt_tokens}, completion: {usage.completion_tokens}, total: {usage.total_tokens}"
    except ArcLLMError as e:
        return False, str(e)
...
def main():
    ...
    test_models = [
        m for m in ANTHROPIC_MODELS 
        if "latest" in m.model_id or m.model_id in [
