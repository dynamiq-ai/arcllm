"""
SambaNova adapter for arcllm.

SambaNova Cloud serves Llama, Qwen and DeepSeek models behind an
...
__all__ = ["SambaNovaAdapter"]


class SambaNovaAdapter(OpenAIAdapter):
    """Adapter for SambaNova Cloud API (OpenAI-compatible)."""

    provider_name = "sambanova"

    def __init__(self, config: ProviderConfig) -> None:
        super().__init__(config)
        self._api_base = config.api_base or "https://api.sambanova.ai/v1"

    def _build_headers(self) -> dict[str, str]:
        api_key = self._get_api_key("SAMBANOVA_API_KEY")
        headers = {
    ...
    def build_embedding_request(
        ...
        **kwargs: Any,
        ...
        raise UnsupportedModelError(
            "SambaNova does not provide an embeddings API",
...
register_provider("sambanova", SambaNovaAdapter)
