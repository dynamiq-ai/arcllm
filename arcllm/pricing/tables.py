...
def cost_per_token(
    ...
    *,
    ...
    if reasoning_rate is not None and reasoning_tokens > 0:
        ...
        completion_cost = (non_reasoning / 1_000_000) * output_rate + (
...
def image_cost(model: str, n: int = 1) -> float:
    """USD cost for ``n`` images generated by ``model``.

    Looks up :attr:`ModelPricing.image_per_request` and multiplies by
    the image count. Raises :class:`UnknownModelPricingError` when the
    model isn't configured for image billing (e.g. chat models).
    """
    pricing = get_model_pricing(model)
    if pricing.image_per_request is None:
        raise UnknownModelPricingError(
            f"No per-image pricing configured for {model!r}",
            model=model,
        )
    return pricing.image_per_request * n
...
def audio_cost(
    ...
    *,
    ...
    """USD cost for an audio call.

    Pass ``characters`` for TTS (per-character billing) or ``seconds``
    for STT (per-second billing). Exactly one of the two must be set
    and the model must have the matching pricing field configured. Raises
    :class:`UnknownModelPricingError` otherwise.
    """
    pricing = get_model_pricing(model)
    if characters is not None and pricing.audio_per_character is not None:
        return pricing.audio_per_character * characters
    if seconds is not None and pricing.audio_per_second is not None:
        return pricing.audio_per_second * seconds
    raise UnknownModelPricingError(
        f"No matching audio pricing configured for {model!r} "
        f"(characters={characters}, seconds={seconds})",
        model=model,
    )


def rerank_cost(model: str, queries: int = 1) -> float:
    """USD cost for a rerank call with ``queries`` searched batches.

    Cohere, Vertex Reranker, and similar providers charge a flat fee
    per query (regardless of how many documents are passed in). Raises
    :class:`UnknownModelPricingError` when the model isn't configured
    for rerank billing.
    """
    pricing = get_model_pricing(model)
    if pricing.rerank_per_query is None:
        raise UnknownModelPricingError(
            f"No per-query rerank pricing configured for {model!r}",
            model=model,
        )
    return pricing.rerank_per_query * queries
